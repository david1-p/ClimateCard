...
public class StationController {
     ...
     * @param radius 반경 (50m ~ 2000m, 기본값: 100m)
     */
    ...
    public ResponseEntity<List<StationResponse>> searchStations(
        ...
        if (keyword == null || keyword.trim().isEmpty() || keyword.length() > 50) {
            throw new IllegalArgumentException("검색어는 1자 이상 50자 이하여야 합니다");
        }

        log.debug("정류소 검색 요청 - 검색어: {}", keyword);

        List<StationResponse> stations = stationService.findStationsByName(keyword.trim());
        return ResponseEntity.ok(stations);
    }
